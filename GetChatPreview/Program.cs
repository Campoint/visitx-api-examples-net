...
namespace Campoint.Visitx.API.Samples.GetChatPreview
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var chatPreviewUrl = await GetChatPreview(availableChatPartner);

            Console.WriteLine("Chat Preview Url:");
            Console.WriteLine(chatPreviewUrl);
        ...
        private static async Task<string> GetChatPreview(dynamic availableChatPartner)
        {
            // get the sender's id
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                var uri = $"https://visit-x.net/interfaces/content/instant.php?userID={senderId}";
